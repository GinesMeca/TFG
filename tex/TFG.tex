...
Veamos en primer lugar cuál es la Loss Function escogida. Notemos que en el caso de clasificación binaria nuestro conjunto de datos se corresponderá con una muestra aleatoria simple de una población con distribución bernouilli. Es decir:
\begin{equation*}
f(y, p) = p^y(1-p)^{1-y}
\end{equation*}
...
No obstante, ésta es una función de $p$ y, como hemos avanzado antes, deseamos trabajar con $log-odd-ratio$. Entonces, transformamos esta expresión de manera que las $p$ pasen a ser $log-odd-ratio$:
\begin{equation*}
\begin{split}
L(y_i, p) &= -(y_i ln(p) + (1-y_i)ln(1-p)) = -y_i ln(p) - ln(1-p) + y_i ln(1-p) = \\
& = -y_i(ln(p) - ln(1-p)) - ln(1-p) = -y_i ln \left( \frac{p}{1-p} \right) - ln(1-p) = \\
& = -y_i ln(odd-ratio) - ln(1-p)
\end{split}
\end{equation*}

Además, se puede comprobar que $p = \frac{e^{ln(odd-ratio)}}{1 + e^{ln(odd-ratio)}}$ y así:
\begin{equation*}
ln(1-p) = ln \left( 1- \frac{e^{ln(odd-ratio)}}{1 + e^{ln(odd-ratio)}} \right) = ln \left( \frac{1}{1 + e^{ln(odd-ratio)}} \right) = - ln \left( 1 + e^{ln(odd-ratio)} \right)
\end{equation*}

Por tanto:
\begin{equation*}
L(y_i, p) = -y_i ln(odd-ratio) - ln(1-p) = -y_i ln(odd-ratio) + ln \left( 1 + e^{ln(odd-ratio)} \right)
\end{equation*}

\noindent
y ya tenemos la Loss Function en función de $log-odd-ratio$:
\begin{equation*}
L(y_i, log-odd-ratio) = -y_i ln(odd-ratio) + ln \left( 1 + e^{ln(odd-ratio)} \right)
\end{equation*}
...
& \Leftrightarrow \, \, p = \frac{\sum_{i=1}^{N} y_i}{N} \, \, \Rightarrow  \, \, f_0(x) = \frac{\sum_{i=1}^{N} y_i}{N}
\end{split}
\end{equation*}

Podemos observar, dado que $y_i \in \{0, 1 \} \, \forall i \in \{1, \dots, n \}$, que $f_0(x)$ es la proporción de individuos del conjunto de entrenamiento que pertenecen a la clase '1'.
